...
export const SovereigntyPanel: React.FC = () => {
  ...
  const { covenant, resources } = state;

  const getRecommendation = () => {
    const { prata } = resources;
    const { loyalty, publicHealth, security } = covenant;

    if (prata < 35) {
      return "Prata baixa! Estimule o Comércio Local ou envie caravanas para gerar receita.";
    }
    if (loyalty < 40) {
      return "Lealdade instável. Uma ação de Governança acalmará os camponeses.";
    }
    if (publicHealth < 40) {
      return "Saúde pública em alerta. Promova ações profiláticas e sanitárias.";
    }
    if (security < 35) {
      return "Segurança vulnerável. Considere patrulhar as estradas ou desafiar rivais.";
    }
    return "A soberania está estável. Favorece estudos na Biblioteca ou projetos de Laboratório.";
  };

  const getHealthStatus = (val: number) => {
    if (val < 35) return "Crítica";
    if (val < 45) return "Alerta";
    return "Boa";
  };

  const getSecurityStatus = (val: number) => {
    if (val < 30) return "Crítica";
    if (val < 40) return "Alerta";
    return "Estável";
  };
  ...
  return (
    <div className="bg-arkanus-panel border border-arkanus-border flex flex-col h-full rounded shadow-lg overflow-hidden relative group">
      ...
      <div className="flex-1 p-3 flex flex-col gap-4">
        {/* Map Image container */}
        <div className="relative w-full flex-1 min-h-[220px] rounded border border-arkanus-border-highlight overflow-hidden bg-stone-900 group">
        ...
        <div className="grid grid-cols-2 gap-3">
            <StatBox icon={<Coins className="w-5 h-5 text-yellow-500/80" />} title="Renda" value={`+${covenant.incomePerSeason} /est.`} sub={`${Math.floor(resources.prata)} Prata`} valueColor="text-arkanus-green" />
            <StatBox icon={<Scale className="w-5 h-5 text-amber-600/80" />} title="Despesas" value={`-${covenant.expensesPerSeason + (state.laboratory.level * 10)} /est.`} sub={`Lab Lvl ${state.laboratory.level}`} valueColor="text-arkanus-red" />
            <StatBox icon={<Droplets className="w-5 h-5 text-green-500/80" />} title="Saúde Pública" value={`${covenant.publicHealth}%`} sub={getHealthStatus(covenant.publicHealth)} valueColor="text-arkanus-gold-light" />
            <StatBox icon={<ShieldCheck className="w-5 h-5 text-stone-300/80" />} title="Segurança" value={`${covenant.security}%`} sub={getSecurityStatus(covenant.security)} valueColor="text-arkanus-gold-light" />
        </div>
        
        {/* Council Recommendation */}
        <div className="mt-2 bg-[#17120e] border border-[#ce9c56]/20 rounded p-2.5 shadow-inner">
           <span className="text-[9px] uppercase tracking-wider text-[#ce9c56] font-display font-semibold block mb-0.5">Conselho Arcano:</span>
           <p className="text-[11px] leading-relaxed text-arkanus-text-dim font-serif italic">
             "{getRecommendation()}"
           </p>
        </div>
      </div>
